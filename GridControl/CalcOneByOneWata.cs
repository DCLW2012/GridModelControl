    ...
    public class CalcOneByOneWata
    {
        ...
        public static bool runBySingleCC()
        {
            ...
            for (int d = 0; d < datnum; ++d)
            {
                ...
                if (dbTableConfigs["china"].Count > 0)
                {
                    ...
                    for (int g = 0; g < processGroup; ++g)
                    {
                        ...
                        while (pids.Count > 0)
                        {
                            ...
                            Console.WriteLine(string.Format("等待第{0}场{1}文件的第{2}进程组计算完成并关闭，pid进程查询更新等待中，等待时长30秒...", d+1, curDatFullname, g) + DateTime.Now);
                ...
                {
                    //! 执行等待，然后查询更新pids列表.等待1分钟
                    Console.WriteLine(string.Format("等待第{0}场{1}文件计算完成并关闭，pid进程查询更新等待中，等待时长30秒...",d+1, curDatFullname) + DateTime.Now);
                    System.Threading.Thread.Sleep(1000 * 30 * 1);
                    perWaitCount++;
